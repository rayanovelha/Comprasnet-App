...
class ForgotPasswordFragment : Fragment() {
    ...
    fun irParaLogin(v: View) {
        ...
        val nome = binding.TextEditForgotName.text.toString().trim()
        val password = binding.TextEditConfirmPassword.text.toString().trim()

        if (verificaCampos()) {
            lifecycleScope.launch(IO) {
                withContext(Main) {
                    binding.btSendEmailForgot.isEnabled = false
                }
    ...
    private suspend fun updateUser(emailInput: String, nome: String, passwordInput: String): Boolean {
    ...
    fun verificaCampos(): Boolean {
        with(binding) {
            ...
            if ((!password1.isEmpty() && !password2.isEmpty()) && (password1 != password2)) {
                Toast.makeText(context, "As duas senhas digitadas não conferem", Toast.LENGTH_SHORT).show()
                TextEditConfirmPassword.requestFocus()
                return false
            }
            if (!email.isEmpty() && password1.isEmpty()) {
                Toast.makeText(context, "Você precisa mudar sua senha, para mudar o email", Toast.LENGTH_LONG).show()
